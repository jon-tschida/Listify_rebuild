...
export default function SearchRecipes(props) {
  ...
  React.useEffect(() => {
    setApiUrl(
      `https://api.edamam.com/api/recipes/v2?type=public&q=${formData}&app_id=c0d5554f&app_key=${process.env.API_KEY}&imageSize=SMALL&field=label&field=image&field=images&field=ingredientLines&field=ingredients&field=source&field=url`
    );
  }, [formData]);
  return (
    <div className="w-screen h-screen bg-black/80">
      <div className="absolute flex flex-col -translate-x-1/2 -translate-y-1/2 rounded-lg laptop:w-3/5 sm-phone:w-full bg-mealsAndListBg drop-shadow-lg laptop:h-4/5 sm-phone:h-3/5 left-1/2 top-1/2">
        <Image
          priority
          alt="close"
          src={closeIcon}
        ...
        <div className="flex flex-col items-center max-h-full overflow-auto">
          {recipeComponents.map((recipeComponent, index) => (
            <div
              key={index}
              className="flex sm-phone:w-5/5 sm-phone:min-h-[150px] laptop:min-h-[200px] p-2 m-2 rounded-md bg-stone-100 drop-shadow-lg laptop:w-5/6"
            >
              {recipeComponent}
            </div>
          ))}
